...
var Variable = {
    ...
    'Temperature':{
        ...
        SetTemperature: function(temp)
        {
            ...
            return temperatureVar.code + temperatureHex;
        }
    },
    'Time':{
        key: 'time',
...
var StatusWord = {
    ...
    '14':{  //工作模式
        ...
        Parse: (value)=>{
            ...
            var status = {
                [Driver.StatusWords.mode.word]: mode
            };
            return status;
        }
    },
    ...
    '9C':{  //面板开关状态
        ...
        Parse: (value)=>{
            ...
            return {
                [Driver.StatusWords.switch.word]: status,
            }
        }
    },
    '9D':{  //面板锁定状态
...
exports.TranslateValue = function (driverName, command, funcid, value)
{
    ...
    switch(funcid){
        ...
        {
            try {
                status = StatusWord['14'].Parse(value);
            }
            catch(e){
                status = {};
            }
        ...
        {
            try {
                status = StatusWord['9C'].Parse(value);
            }
            catch(e){
                status = {};
            }
...
function EMCViewStatus(ext, param)
{
    ...
    switch(param.mode){
        ...
        case Driver.Command.EMC_SCALE:
        {
            ...
            deviceAddr = deviceAddr.reverse();
            deviceAddr = Driver.ToString(deviceAddr);
            var adaptDevice = ext && ext.adaptdevice;
            if (!adaptDevice) {
                return {
                    code: ErrorCode.Code.DEVICETYPEUNSUPPORT,
                    message: ErrorCode.Message.DEVICETYPEUNSUPPORT
